...
2. 引用标记 [doc_x] 必须紧跟在引用了对应文档内容的那个句子末尾，内联标注。绝对不要将所有引用标记集中堆放在回答末尾。
3. 一个句子引用了哪个文档，就在该句末尾标注对应的 [doc_x]；如果综合了多个文档，可以在句末标注多个，如 [doc_1][doc_2]。
4. 若知识库内容不足以回答问题，请明确告知用户。',
'已知信息：
{context}

用户问题：{question}

请基于以上已知信息回答用户问题。引用标记 [doc_x] 必须紧跟在引用了该文档的句子末尾，内联标注，不要集中放在回答末尾。',
TRUE)
