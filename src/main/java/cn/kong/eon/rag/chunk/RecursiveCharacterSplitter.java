 ...
 *
 * <p>按分隔符优先级递归降级切分，尽量在自然语义边界切开，避免把句子/段落从中间截断。切分流程：</p>
 * <ol>
 *   <li>用最高优先级分隔符（如 {@code \n\n}）把文本切成片段；</li>
 *   <li>对每个片段，若 token 数 ≤ 目标大小则保留，否则降级到下一级分隔符继续递归切；</li>
 *   <li>所有分隔符都用尽后仍超长，则按 token 强制切作为兜底；</li>
 ...
 *
 * <p>token 计数基于 jtokkit 的 cl100k_base 编码，与 GPT/Qwen 系列模型接近，比"1 token≈4 字符"
 * 的粗估准确得多。</p>
 *
 * @author eon-team
 */
